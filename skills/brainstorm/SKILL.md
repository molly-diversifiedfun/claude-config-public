---
name: brainstorm
description: Multi-session, document-versioned brainstorming partner — for ideation projects that span days or weeks, where session continuity matters. Use when the user says "brainstorm doc", "brainstorming session", "continue brainstorm", "pick up the brainstorm", "open a new brainstorm", "brainstorm a book/newsletter/series/launch", "long-running ideation", or asks to think through a problem with a persistent artifact across sessions. NOT for single-turn ideation or pre-build feature dialogue ("let's brainstorm this feature" / "explore approaches" / "think through this") — those go to the plugin variants (superpowers:brainstorming, compound-engineering:brainstorming). The differentiator: this skill versions a markdown document and tracks decisions across days.
---
