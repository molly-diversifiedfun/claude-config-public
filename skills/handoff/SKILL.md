...
### Step 0: End-of-Session Checklist (MANDATORY — do this BEFORE the handoff)

You MUST complete this checklist before generating the handoff document. Do NOT skip to "Assess Session State." Present results to the user.

**Retrospective:**
1. What went wrong this session that I should do differently next time?
2. What worked well that I should keep doing?
3. What process gap did I discover?

**Save findings:**
4. Save retrospective to memory as a `feedback_session{N}_learnings.md` file
5. Add any new skill/rule/tool needs to TASKS.md under discovered work

**Verify completeness:**
6. Run `./scripts/doc-freshness.sh` — are all docs fresh?
7. Run `npx vitest run 2>&1 | tail -5` — do all tests pass?
8. Check `git status` — is anything uncommitted?
9. Check for missing ADRs — did you make architectural decisions without writing ADRs?
10. Is CLAUDE.md current? (test count, edge function count, table count)

**Present the checklist results to the user as a pass/fail table.** Fix any failures before proceeding to Step 1.
