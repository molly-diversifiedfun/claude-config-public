...
If BOTH are confidently inferable, state them inline before scoring:

> Auditing [artifact] for **[audience]** who needs to **[intent]**.

Then proceed to the 5 rungs.

### Ask path (when confused)

If either audience or intent is unclear, STOP and ask via AskUserQuestion.
Do not score until answered.

- Ambiguous audience signals: artifact has no audience markers, multiple
  possible audiences (team vs. customers vs. contributors), or "users"
  unspecified.
- Ambiguous intent signals: no clear call-to-action, multiple target
  behaviors possible, success criteria not stated.

Example AskUserQuestion when both are unclear:

> Q1: Who is the audience for this audit?
>   Options: [list 3-4 plausible audiences inferred from context], Other
> Q2: What does "follow through" mean here? (the target behavior)
>   Options: [list 3-4 plausible target behaviors], Other

When only ONE is unclear, ask just that one.

### Why this exists

A followability score without explicit audience + intent is theatre. The
same artifact scores 5/5 for one audience and 0/5 for another. State the
frame before the score, or refuse to score.

## The 5 rungs

Score against the audience + intent you confirmed in Step 0. If you skipped Step 0, stop and do it now.
...
- The full methodology lives in The Followable System by you Shelestak. The Color Check lead magnet (`launch/platform/followability-audit-lead-magnet.md` in the source project) is the canonical short-form.

---

## Provenance

This skill packages The Followable System's "Color Check" — the diagnostic at the heart of the book by you Shelestak. The 5-rung model (Understand / Believe / Do / Repeat / Share) is the structural innovation: the system's progression matches the human's journey.
