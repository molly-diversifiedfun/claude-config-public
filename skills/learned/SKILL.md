...
Cross-project learnings synthesized from session feedback. 16 patterns extracted from 80+ memory files across 11 projects in the original config, representing corrections the original author made repeatedly. Two former entries (`competitive-history.md` and `hook-performance.md`) were moved out of `learned/` to the memory/ directory before this public snapshot — they were rolling data, not patterns.
